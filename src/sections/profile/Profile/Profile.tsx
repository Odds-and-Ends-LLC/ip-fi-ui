  ...
  return (
    ...
    >
      <Stack className={styles.profileBanner} sx={{ bgcolor: color }}>
        {!loading && (
          <>
            <Box className={styles.profileBannerShadow} sx={{ bgcolor: "background.black" }} />
      ...
      >
        ...
        <Stack className={styles.profileSectionName}>
          <Stack>
            ...
            {user.lastActiveAt && (
              <Typography variant="body2" color="text.disabledBlue">
                Active {formatDistanceToNow(user.lastActiveAt, { addSuffix: true })}
              </Typography>
            )}
          </Stack>
        ...
        />
        {user.about && <CollapsibleText>{user.about}</CollapsibleText>}
        <InfoList
          info={[
            {
              label: "Joined",
