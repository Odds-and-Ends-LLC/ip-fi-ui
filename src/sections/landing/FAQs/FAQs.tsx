...
export default function FAQs() {
  ...
  return (
    ...
    >
      <Image
        src="/images/faqs_cover_image.png"
          ...
          >
            <AccordionSummary>
              <Stack className={styles.faqsNumber}>
                <Typography variant="button1" color="text.brandSecondary">{(i + 1).toString().padStart(2, "0")}</Typography>
              </Stack>
              {q}
            </AccordionSummary>
            <AccordionDetails  className={styles.faqsAnswer}>
              <Typography variant="body1" color="text.secondary">{a}</Typography>
            </AccordionDetails>
          </Accordion>
        ))}
      </Stack>
